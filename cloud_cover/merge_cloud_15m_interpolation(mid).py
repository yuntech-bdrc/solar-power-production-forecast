...
# # <font color=#0000FF>每10分鐘雲資料重取樣為每15分鐘一筆資料，此外將low為NaN的值用mid來填補</font>
...
# 產生空的df2 
df2 = pd.DataFrame({'date_time': dates, 'lon': 120.560722, 'lat': 24.066583, 'low': np.nan, 'mid': np.nan, 'hig': np.nan, 'cloud': np.nan})

# 將df中的資料複製到df2中
df2.loc[df2['date_time'].isin(df.index), ['low','mid', 'hig' ,'cloud']] = df[['low','mid', 'hig','cloud']].values

# 對df2進行線性插值
df2[['mid', 'hig','cloud']] = df2[['mid', 'hig','cloud']].interpolate()
